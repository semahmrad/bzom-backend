    ...
    try {
        ...
        if (
            ...
            if (imgSizeInMegabytes > 10) {
                ...
                console.log(
                    "size before compress => " + imgData.toString().length
                );
                let minibuffer = await compressImg(imgData);
                console.log(
                    "size after compress => " + minibuffer.toString().length
                );
                ...
                await newUser
                    ...
                    .then((user) => {
                        ...
                        sign(
                            payload,
                            "SRKxbUrUC8H3v4%*shuATBkZwhCQ^p=&6G!F8pz4+Jv*&Gg&a_v=68p2DXcHjq8mk_sMRdF2K#Yf3GE_@Hd@*$QWyQ3_h#5uYG%^P$J--nE_bL-NsE!sx@^",
                            {},
                            async (err, token) => {
                                if (err) {
                                    res.json({
